<!-- C:\xampp\htdocs\projetweb-test\view\adhesion\create.php -->
<?php if (!isset($_GET['controller']) || !isset($_GET['action'])) {
    header('Location: ../../index.php?controller=adhesion&action=create');
    exit;
} ?>
<!DOCTYPE html>
<html lang="fr">
...
<body>
    <div class="container mt-5">
        <h1 class="mb-4">Ajouter une adhésion</h1>

        <?php if (isset($error)): ?>
            <div class="alert alert-danger"><?= htmlspecialchars($error) ?></div>
        <?php endif; ?>

        <form action="?controller=adhesion&action=store" method="POST" class="needs-validation" novalidate>
            <div class="mb-3">
                ...
                <select class="form-control" id="id_etudiant" name="id_etudiant" required>
                    ...
                    <?php foreach ($etudiants as $etudiant): ?>
                        <option value="<?= $etudiant->getIdEtudiant() ?>"><?= htmlspecialchars($etudiant->getNom() . ' ' . $etudiant->getPrenom()) ?></option>
                    <?php endforeach; ?>
                </select>
            ...
            <div class="mb-3">
                ...
                <select class="form-control" id="id_club" name="id_club" required>
                    ...
                    <?php foreach ($clubs as $club): ?>
                        <option value="<?= $club->getIdClub() ?>"><?= htmlspecialchars($club->getNomClub()) ?></option>
                    <?php endforeach; ?>
                </select>
